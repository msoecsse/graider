...
enum WorkspaceTestNumber {
  /** 10 MiB; a full template clone's Git output does not fit the default buffer. */
  GitOutputMaxBufferBytes = 10485760,
  LastArgumentIndex = -1
}
...
const runGit = async (directory: string | undefined, args: string[]): Promise<string> => {
  const { stdout } = await execFile(
    ...
    { maxBuffer: WorkspaceTestNumber.GitOutputMaxBufferBytes }
  );
  return stdout.trim();
};
        ...
        },
        {
          runGit: async (directory, args) => {
            ...
            if (args[0] === "clone") {
              cloneCount += 1;
              if (cloneCount === 2)
                studentDirectory = args.at(WorkspaceTestNumber.LastArgumentIndex);
            }
