...
publish artifact-based faculty-provided student reports, and run no-grading
assignments through validate/apply/grade/report/publish paths.
...
Automated grading must be optional. No-grading behavior:
...
workflow generate -> fails clearly with workflow_generation_not_configured
```
...
report publishing. Published reports must contain only the target student's
information and must be written only to that student's repository.
...
parsing or modifying the content. The current runtime support is artifact-based:
the configured `source_file` must exist inside the configured artifact.
...
configured destinations. The faculty-provided report source is read from the
configured artifact.
...
- reports clear diagnostics if the artifact or source file is missing
...
`graider validate` performs pragmatic static workflow compatibility checks for
grading-enabled assignments. It verifies that the configured workflow path can be
found locally and that the workflow declares `workflow_dispatch`. For preset
assignments, validation also recognizes the local workflow generated by
`graider workflow generate` under
`terms/<term>/generated-workflows/<assignment>/grade.yml`.

For custom and contract-only workflows, validation does not try to prove every
grading implementation detail. It does not hard-fail solely because it cannot
statically confirm an `upload-artifact` step, the exact artifact name, the exact
result file write, or the semantic correctness of tests. Runtime report
diagnostics remain responsible for missing artifacts, missing result files, and
...
The helper writes valid JSON with `schema_version: 1`, named checks, and an
overall status. It is not a published GitHub Action; it is embedded into
generated workflows as ordinary Python using only the Python standard library.

Copyable faculty-facing examples for presets, custom workflows, contract-only
workflows, GitHub Classroom integration, faculty-provided reports, and
...
- faculty-provided report artifact missing
- faculty-provided report source file missing
...
- extend faculty-provided report publishing sources
- expand reusable grading examples as new presets and publishing sources are
  added
