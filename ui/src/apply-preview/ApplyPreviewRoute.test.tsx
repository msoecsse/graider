...
describe("ApplyPreviewRoute", () => {
  it("renders a breadcrumb trail with Apply as the current page and the assignment title linking back", async () => {
    mockGraiderUI();
    ...
    expect(within(breadcrumb).getByText("Apply")).toHaveAttribute("aria-current", "page");
  });

  it("navigates back to assignment detail via the assignment breadcrumb", async () => {
    mockGraiderUI();
    renderAtRoute(`/course/csc1120/27s1/lab02/apply`);
  ...
  it("shows RouteNotFound when the assignment slug is unresolvable", async () => {
    mockGraiderUI();
    renderAtRoute("/course/csc1120/27s1/does-not-exist/apply");
